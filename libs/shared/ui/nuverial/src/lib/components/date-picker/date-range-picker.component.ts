...
export class NuverialDateRangePickerComponent extends FormInputBaseDirective implements ControlValueAccessor, OnInit {
  ...
  public valueChanged() {
    ...
    const newStart = datePipe.transform(this.startDate.value, 'yyyy-MM-dd');
    const newEnd = datePipe.transform(this.endDate.value, 'yyyy-MM-dd');
    this.startDate.setValue(newStart);
    this.endDate.setValue(newEnd);

    this.formControl.setValue({ endDate: this.endDate.value, startDate: this.startDate.value });
  }
